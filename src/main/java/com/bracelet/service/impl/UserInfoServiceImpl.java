...
public class UserInfoServiceImpl implements IUserInfoService {
	...
	@Override
	public BusinessUserInfo getBusinessUserInfoByUsername(String userName) {
		String sql = "select * from business_user_info where  username=?  LIMIT 1";
		List<BusinessUserInfo> list = jdbcTemplate.query(sql, new Object[] { userName },
				new BeanPropertyRowMapper<BusinessUserInfo>(BusinessUserInfo.class));
		if (list != null && !list.isEmpty()) {
			return list.get(0);
		} else {
			logger.info("cannot find userinfo,imei:" + userName);
		}
		return null;
	}

	@Override
	public boolean updateUserBalanceById(Integer id, Integer balance) {
		Timestamp now = Utils.getCurrentTimestamp();
		int i = jdbcTemplate.update("update business_user_info set balance=? , updatetime=? where id = ?", new Object[] { balance, now, id },
				new int[] { Types.INTEGER, Types.TIMESTAMP, Types.INTEGER });
		return i == 1;
	}

	@Override
	public List<BusinessUserInfo> getBusinessUserListInfo(Integer start, Integer end, Integer status) {
		String sql = "select * from business_user_info order by id desc limit  "+start +","+end; //where b_imei=?
		if(status!=3){
			sql = "select * from business_user_info where use_status=?  order by id desc limit  "+start +","+end;
		}
		List<BusinessUserInfo> list = jdbcTemplate.query(sql, new Object[] { status },
				new BeanPropertyRowMapper<BusinessUserInfo>(BusinessUserInfo.class));
		if (list != null && !list.isEmpty()) {
			return list;
		} else {
	...
	@Override
	public boolean updateStatusInfoById(Long id, Integer status, String userName) {
		Timestamp now = Utils.getCurrentTimestamp();
		int i = jdbcTemplate.update("update business_user_info set use_status=? , updatetime=? where id = ?", new Object[] { status, now, id },
				new int[] { Types.INTEGER, Types.TIMESTAMP, Types.INTEGER });
		return i == 1;
	}

	@Override
	public BusinessUserInfo getBusinessUserInfoById(Integer id) {
		String sql = "select * from business_user_info where  id=?  LIMIT 1";
		List<BusinessUserInfo> list = jdbcTemplate.query(sql, new Object[] { id },
				new BeanPropertyRowMapper<BusinessUserInfo>(BusinessUserInfo.class));
		if (list != null && !list.isEmpty()) {
			return list.get(0);
		} else {
			logger.info("cannot find userinfo,imei:" + id);
		}
		return null;
	}
