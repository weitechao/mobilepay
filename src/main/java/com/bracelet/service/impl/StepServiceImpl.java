...
public class StepServiceImpl implements IStepService {
	...
	@Override
	public boolean insertOrderErrorInfo(String userName, String orderId, Integer orderStatuInt, String orderStatuText) {
		Timestamp now = Utils.getCurrentTimestamp();
		int i = jdbcTemplate.update("insert into order_error_info (username, order_id, order_statu_int,order_statu_text, createtime) values (?,?,?,?,?)",
				new Object[] { userName, orderId, orderStatuInt, orderStatuText, now},
				new int[] { Types.VARCHAR, Types.VARCHAR, Types.INTEGER, Types.VARCHAR, Types.TIMESTAMP });
		return i == 1;
	}
}
