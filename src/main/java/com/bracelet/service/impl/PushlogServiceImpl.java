...
public class PushlogServiceImpl implements IPushlogService {
	...
	public PhoneCharge getCharge(String phone) {
		String sql = "select * from phone_balance where phone=?  LIMIT 1";
	...
	@Override
	public boolean updateChargeBalanceById(Long id, Integer balance) {
		Timestamp now = Utils.getCurrentTimestamp();
		int i = jdbcTemplate.update(
				"update phone_balance set balance = ? , updatetime = ? where id = ?",
				new Object[] { balance, now , id},
				new int[] { Types.INTEGER, Types.TIMESTAMP, Types.INTEGER });
		return i == 1;
	}

	@Override
	public boolean insertPhoneBalance(String chargeAcct, Integer chargeCash) {
		Timestamp now = Utils.getCurrentTimestamp();
		int i = jdbcTemplate.update(
				"insert into phone_balance (phone, balance, updatetime, createtime) values (?,?,?,?)",
				new Object[] { chargeAcct, chargeCash, now ,now}, new int[] {
