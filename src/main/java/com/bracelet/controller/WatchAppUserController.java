...
public class WatchAppUserController extends BaseController {
	...
	public String getbindDeviceList(@PathVariable String token) {
		...
		List<BindDevice> bdList = userInfoService.getBindInfoById(Long.valueOf(token));
		...
		if (bdList != null) {
			...
			UserInfo UserInfo = userInfoService.getUserInfoById(Long.valueOf(token));
			if (userInfoService.saveBindInfo(Long.valueOf(token), UserInfo.getUsername(), "", 0)) {
				List<BindDevice> bdListt = userInfoService.getBindInfoById(Long.valueOf(token));
	...
	//获取绑定列表
		...
		/*	BindDevice  bindDeviceApp= userInfoService.getWatchBindInfoByUserId(Long.valueOf(userId));
			...
			List<BindDevice> list = userInfoService.getWatchBindInfoByBimei(token);
			JSONArray jsonArray = new JSONArray();
